...
void USART2_IRQHandler(void)
{
  /* USER CODE BEGIN USART2_IRQn 0 */
	uint32_t errorflags = 0x00U;
	/* If no error occurs */
	errorflags = (USART2->SR & (uint32_t)(USART_SR_PE | USART_SR_FE | USART_SR_ORE | USART_SR_NE));
	if(errorflags == 0)
	{
		/* UART in mode Receiver -------------------------------------------------*/
		if(((USART2->SR & USART_SR_RXNE) != 0) && ((USART2->CR1 & USART_CR1_RXNEIE) != 0))
		{
			...
			{
				uart_rx_char = USART2->DR; // Lost char
				return;
			}

			uart_rx_pointer_w = (uart_rx_pointer_w + 1) & 1023;

			uart_rx_bufer[uart_rx_pointer_w] = USART2->DR;
			return;
		}
	}
	else
